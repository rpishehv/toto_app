  ...
  return(
    <div style={{minHeight:"100vh",background:"#0a0d12",
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"16px 12px"}}>
        ...
        {/* ── GROUP ANALYTICS (separate from stats IIFE) ── */}
        {tab==="stats"&&(
          <div style={{padding:"0 16px 24px"}}>
            ...
            {!groupAnalytics&&!analyticsLoading&&(
              <div style={{textAlign:"center",padding:"32px 20px",color:"#444"}}>
                <div style={{fontSize:32,marginBottom:8}}>🔍</div>
                <div style={{fontSize:12,marginBottom:4}}>No analysis yet</div>
                <div style={{fontSize:11,color:"#333"}}>Generate once matches have been played</div>
              </div>
            )}

            {groupAnalytics&&!Array.isArray(groupAnalytics)&&(()=>{
              const a = groupAnalytics;
              if(!a.headline && !a.player_profiles?.length) return(
                <pre style={{fontSize:10,color:"#555",whiteSpace:"pre-wrap",wordBreak:"break-all"}}>
                  {JSON.stringify(a,null,2)}
                </pre>
              );
              return(
                <div>
                  {a.headline&&<div style={{padding:"12px 16px",borderRadius:10,marginBottom:12,
                    background:"linear-gradient(135deg,rgba(139,92,246,0.12),rgba(139,92,246,0.06))",
                    border:"1px solid rgba(139,92,246,0.25)"}}>
                    <div style={{fontSize:14,fontWeight:700,color:"#c4b5fd"}}>{a.headline}</div>
                  </div>}
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
                    {[
                      {label:"🏆 Leader",value:a.leader_analysis,color:"#fcb900"},
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:4}}>
                    {a.prediction&&<div style={{padding:"10px 14px",borderRadius:8,background:"rgba(252,185,0,0.06)",border:"1px solid rgba(252,185,0,0.2)"}}>
                      <div style={{fontSize:10,color:"#fcb900",fontWeight:700,marginBottom:4}}>🏆 Who wins this?</div>
                      <div style={{fontSize:11,color:"#888",lineHeight:1.5}}>{a.prediction}</div>
                    </div>}
                    {a.banter&&<div style={{padding:"10px 14px",borderRadius:8,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)"}}>
                      <div style={{fontSize:10,color:"#fb923c",fontWeight:700,marginBottom:4}}>😂 Banter corner</div>
                      <div style={{fontSize:11,color:"#888",lineHeight:1.5,fontStyle:"italic"}}>{a.banter}</div>
                    </div>}
                  </div>
