...
import QF_AI_PREDICTIONS from './qfPredictions.js';
import QF_EXPERT_PREDICTIONS from './qfExperts.js';
...
function KOMatchButtons({liveHome, liveAway, aiP, r32AI, r32Expert, r16AI, r16Expert, qfAI, qfExpert}) {
  const [showAI, setShowAI] = useState(false);
  const [showOdds, setShowOdds] = useState(false);
  const [showExperts, setShowExperts] = useState(false);
...
export default function App(){
  ...
  if (Date.now() > TOURNAMENT_END && tab === 'groups') {
    // Auto-redirect to leaderboard on first load after tournament ends
    setTimeout(() => setTab('leaderboard'), 0);
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0a0d12",
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"16px 12px"}}>
        ...
        {/* ── LEADERBOARD ── */}
        {tab==="leaderboard"&&<div>
          {Date.now() > TOURNAMENT_END && (
            <div style={{marginBottom:20,padding:"16px",borderRadius:12,textAlign:"center",
              background:"rgba(252,185,0,0.06)",border:"1px solid rgba(252,185,0,0.2)"}}>
              <div style={{fontSize:28,marginBottom:6}}>🏆</div>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:2,color:"#fcb900",marginBottom:4}}>
                FIFA World Cup 2026 — Final Standings
              </div>
              {actualPodium?.first&&(
                <div style={{fontSize:13,color:"#888"}}>
                  🥇 {FLAGS[actualPodium.first]||""} {actualPodium.first} · World Champions
                </div>
              )}
            </div>
          )}
